...
dir()
print("this is for the Group 1.5 sequences for Data Matrix")

myAlign<-read.alignment(file = "fastafileSub1.5.fasta", format = "fasta")
...
for (i in 1:length(viruAln$nam)) {
  ...
  assign(name, seq)
  #A epitope 	
  a<-substr(seq, 1, 648)#changed from myAlign table
  b<-substr(seq, 679, 1460)
  A<-paste0(a, b)
  assign(name, A)
  if(i == 1){
    namList<-name}
...
write.csv(myMat, file="myEditFullDM_P1.5.csv")

save.image(file = "/gpfs/fs2/scratch/nporesky/SBM/FullList_edit_strip_DM_P1.5.RData")
...
write.csv(myDF, file = "myMDSP1.5.csv")
